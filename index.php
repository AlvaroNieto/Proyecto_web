    ...
    <body>
      ...
      if (isset($_GET["page"])) {
       ...
       if (isset($_GET['message'])) {
         print '<script type="text/javascript">alert("You need to register and/or loggin for access the cart.");</script>';
         unset($_GET['message']);
       }
            ...
            <div class="col-md-2" id="content">
              ...
              if ($_SESSION['type'] == 'user' or $_SESSION['type'] == 'admin') {
                ...
                <form method="POST" action="index.php">
                  <div class="form-group">
                    <label for="sel3">Selecciona un tema:</label>
                    <select class="form-control" name="estilo" id="sel5">';
                  echo '<option ';
                        $sql="SELECT theme FROM users where 'id' = '".$_SESSION['id']."'";
                        $result = $connection->query($sql);
                        $obj = $result->fetch_object();
                        if ($obj->theme == 'index.css') {
                          echo "selected='selected' ";
                        }
                  echo 'value=index.css>Claro</option>';
                  echo '<option ';
                        $sql="SELECT theme FROM users where `id` = '".$_SESSION['id']."'";
                        $result = $connection->query($sql);
                        $obj = $result->fetch_object();
                        if ($obj->theme == 'index0.css') {
                          echo "selected='selected' ";
                        }
                  echo 'value=index0.css>Gris</option>';
                  echo '<option ';
                          $sql="SELECT theme FROM users where `id` = '".$_SESSION['id']."'";
                          $result = $connection->query($sql);
                          $obj = $result->fetch_object();
                          if ($obj->theme == 'index1.css') {
                            echo "selected='selected' ";
                          }
                  echo 'value=index1.css>Oscuro</option>';
                  echo '</select>
                    <input type="submit" class="btn btn-default" value="Aplicar" style="margin-top: 10px;">
                  </form>
                  </div>';
              }
               ?>
